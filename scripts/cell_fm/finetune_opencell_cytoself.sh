...
# Wandb
export WANDB_RUN_NAME=FT_OC-Cytoself_CELLFM_Dev_NH8_S1_R1
export WANDB_PROJECT=CELL-Diff2
[ -z "${output_dir}" ] && output_dir=finetune_opencell_cytoself/$WANDB_RUN_NAME
...
# Training
[ -z "${vae_loadcheck_path}" ] && vae_loadcheck_path=/hpc/projects/group.huang/dihan.zheng/CELL-Diff2/finetune_opencell/FT_VAE_OC_Cytoself_128_KL1e-4_FP32/checkpoint-50000/pytorch_model.bin
[ -z "${loadcheck_path}" ] && loadcheck_path=pretrain_hpa/PT_HPA_CELLFM_Dev_NH8_all_S1_R2/checkpoint-75000/pytorch_model.bin
...
[ -z "${MASTER_PORT}" ] && MASTER_PORT=29402
...
python -m torch.distributed.run $DISTRIBUTED_ARGS cell_fm/tasks/cell_fm/finetune_opencell_cytoself.py \

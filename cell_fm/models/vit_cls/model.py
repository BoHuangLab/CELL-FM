...
class ViTModel(PreTrainedModel):
    ...
    def prepare_data(self, batched_data):
        ...
        if self.config.cell_image == 'nucl':
            ...
            input_img = torch.cat([nucleus_img, ER_img, protein_img], dim=1)  # [B, 3, H, W]
        elif self.config.cell_image == 'nucl,mt':
            protein_img = batched_data['protein_img']
            nucleus_img = batched_data['nucleus_img']
            microtubules_img = batched_data['microtubules_img']
            input_img = torch.cat([nucleus_img, microtubules_img, protein_img], dim=1)  # [B, 3, H, W]
...
class ViT(VisionTransformer):
    def __init__(self, config: ViTConfig):
        ...
        in_channels = len(config.cell_image.split(',')) + 1

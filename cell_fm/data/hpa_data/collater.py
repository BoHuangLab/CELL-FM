...
def collate_fn(
    ...
    batch["protein_seq"] = torch.cat(
        [
            pad_1d_unsqueeze(
                s["protein_seq"], max_tokens, 0, vocab.pad_token_id
            )
            for s in samples
        ]
    )
    ...
    batch["protein_seq_masked"] = torch.cat(
        [
            pad_1d_unsqueeze(
                s["protein_seq_masked"], max_tokens, 0, vocab.pad_token_id
            )
            for s in samples
        ]
    )
    ...
    batch["location_label"] = torch.cat(
        [s["location_label"].unsqueeze(0) for s in samples]
    )

    batch["protein_idx"] = torch.cat(
        [s["protein_idx"].unsqueeze(0) for s in samples]
    )

...
class OpenCellCropDataset(Dataset):
    def __init__(self, args, split_key) -> None:
        ...
        self.meta_data = pd.read_csv(os.path.join(self.data_path, f'{self.split_key}_2d_proj_256_crop_merged_meta_data.csv'))
...
class OpenCellCropAllImageDataset(OpenCellCropDataset):
    def __getitem__(self, index: int) -> dict:
        meta_data = self.meta_data.iloc[index]
        ...
        for image_path in image_paths:
            protein_img, nucleus_img = self.get_img(image_path)
...
class OpenCellCropImageOnlyDataset(Dataset):
    ...
    def get_img(self, fpath: Path) -> torch.Tensor:
        img = tiff.imread(os.path.join(self.data_path, fpath)).astype(np.float32)
...
class OpenCellCropImageEmbedDataset(Dataset):
    def __init__(self, args, subfolder_name: str):
        super().__init__()
        self.args = args
        ...
        self.meta_data = pd.read_csv(os.path.join(self.data_path, f'all_2d_proj_256_crop_merged_meta_data.csv'))

        self.data_folder_path = self.data_path / subfolder_name

        # get the folder names under data_path as gene names
        self.gene_name_folders = sorted([f.name for f in self.data_folder_path.iterdir() if f.is_dir()])
        self.gene_names = sorted(self.meta_data['gene_name'].unique().tolist())
        ...
        for gene_name_folder in self.gene_name_folders:
            assert (self.data_folder_path / gene_name_folder).is_dir(), f"{gene_name_folder} is not a directory."

            gene_name = gene_name_folder.split('_')[0]
            meta_data = self.meta_data[self.meta_data['gene_name'] == gene_name]
            assert len(meta_data) == 1, f"Gene name {gene_name} has {len(meta_data)} entries in meta data."

            protein_seq = meta_data.iloc[0]['sequence']
            if len(protein_seq) > self.args.max_protein_sequence_len:
                continue

            image_paths = sorted(glob(os.path.join(self.data_folder_path, gene_name_folder, '*.tif')))
            self.img_paths.extend(image_paths)

            locations = meta_data.iloc[0]['locations']
            if pd.isna(locations):
                locations = "NA"

            self.location_labels.extend([locations] * len(image_paths))
    ...
    def __getitem__(self, idx: int) -> dict:
        item = {}
        item['protein_img'], item['nucleus_img'] = self.get_img(self.img_paths[idx])
        item['locations'] = [loc.strip() for loc in self.location_labels[idx].split(',')]
    ...
    def collate(samples):
        ...
        return {"batched_data": {"protein_img": protein_img, "nucleus_img": nucleus_img, "locations": locations}}
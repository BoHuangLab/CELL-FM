...
@cli(ViTConfig, CondenSeqDatasetConfig, ExtendedConfig)
def main(args) -> None:
    # device
    ...
    meta_data = pd.read_csv('/hpc/reference/opencell/condenseq/cellfm_train_meta_data_large_pool_updated.csv')

    # chop the meta_data according to start_index and end_index (not inclusive)
    meta_data = meta_data[start_idx:end_idx]

    data_path = Path('/hpc/reference/opencell/condenseq/seq2img_cellfm_train')
    ...
    for _, row in tqdm(meta_data.iterrows(), total=meta_data.shape[0], desc=f"Processing from {start_idx} to {end_idx} (not inclusive)"):
        ...
        image_path = data_path / f'{protein_index:06d}' / 'imgs.tif'
        protein_intensity_levels = np.load(data_path / f'{protein_index:06d}' / 'protein_intensity_levels.npy')

        save_dir = data_path / f'{protein_index:06d}'

...
def main(args) -> None:
    ...
    data_roots = sorted(glob(f'/hpc/reference/opencell/condenseq/seq2img_single/{protein_name}/{scale}/protein_*'))
    ...
    for data_root in tqdm(data_roots):
        ...
        # uniform jitter
        jitter_strength = 0.1
        y_jittered = y_vals + rng.uniform(-jitter_strength, jitter_strength, size=y_vals.shape)

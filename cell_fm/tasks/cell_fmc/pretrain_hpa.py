...
from cell_fm.data.hpa_data.dataset_with_context import HPADatasetWithContext
...
@cli(CELLFMCConfig)
def main(args) -> None:
    trainset = HPADatasetWithContext(args, split_key=args.split_key)
    model = CELLFMCModel(config=CELLFMCConfig(**vars(args)), loss_fn=ImgDiffCriterion)

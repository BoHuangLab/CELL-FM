...
def main() -> None:
    ...
    # Load FID results
    PUPS = np.load('output/hpa/embed/PUPS_results/fids.npy')
    CD1  = np.load('output/hpa/embed/CELL-Diff_results/fids.npy')
    CD2  = np.load('output/hpa/embed/PT_HPA_CELL-Diff2_Rep_Dev_NH8_S2_R1_50k_results/fids.npy')

    # only keep finite and positive FID values
    ...
    # Describe the filtered FID values
    d1, d2, d3 = describe(PUPS_pos), describe(CD1_pos), describe(CD2_pos)

    # Log-transform the filtered FID values
    ...
    ax.set_xlabel(r'$\log_{10}(\mathrm{msFID\ value})$', fontsize=25)  # CHANGED
    ax.set_ylabel("Number of proteins", fontsize=25)
    ...
    plt.tight_layout()
    hist_path = "output/hpa/embed/fid_histogram.png"
    plt.savefig(hist_path, dpi=300)
    # save to svg
    plt.savefig("output/hpa/embed/fid_histogram.svg", dpi=300, bbox_inches="tight", pad_inches=0, transparent=True)
    plt.close(fig)

    print("FID histogram plot saved to output/hpa/embed/fid_histogram.png and output/hpa/embed/fid_histogram.svg")
    ...
    for i, vals_log in enumerate(data_log):
        ...
        # Write the original space value on the median line (in white)
        ax.text(
            pos[i], med,
            f"{10**med:.3f}",
            color="white",
    ...
    ax.set_ylabel(r'$\log_{10}(\mathrm{msFID\ value})$', fontsize=25)                    # CHANGED
    ax.set_title("msFID Distribution", fontsize=30)

    # Legend (still using the median in the original scale; for geometric median, use 10**np.median(data_log[i]))
    from matplotlib.patches import Patch
    dataset_handles = [
    ...
    # ax.legend(handles=dataset_handles, loc="upper right", frameon=False, fontsize=14, handlelength=2.5, handleheight=1.5)

    plt.tight_layout()
    plt.savefig("output/hpa/embed/fid_violin.png", dpi=300)
    # save to svg
    plt.savefig("output/hpa/embed/fid_violin.svg", dpi=300, bbox_inches="tight", pad_inches=0, transparent=True)
    plt.close(fig)

    print("FID violin plot saved to output/hpa/embed/fid_violin.png and output/hpa/embed/fid_violin.svg")

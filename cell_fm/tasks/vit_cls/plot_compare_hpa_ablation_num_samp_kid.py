...
def main() -> None:
    ...
    # Load KID results
    R0d5k = np.load('output/hpa/embed/PT_HPA_CELL-Diff2_cd2split_0d5k_results/kids.npy')
    R1k   = np.load('output/hpa/embed/PT_HPA_CELL-Diff2_cd2split_1k_results/kids.npy')
    R3k   = np.load('output/hpa/embed/PT_HPA_CELL-Diff2_cd2split_3k_results/kids.npy')
    R6k   = np.load('output/hpa/embed/PT_HPA_CELL-Diff2_cd2split_6k_results/kids.npy')
    Rall  = np.load('output/hpa/embed/PT_HPA_CELL-Diff2_Rep_Dev_NH8_S2_R1_50k_results/kids.npy')
    ...
    ax.set_ylabel("Median msKID", fontsize=30)
    ax.set_title("Median msKID vs. Number of Samples", pad=0, fontsize=35)
    ...
    # Save (publish-ready)
    # -------------------------
    out_base = "output/hpa/embed/median_kid_vs_samples_publish_ready"
    ...
    ax.set_ylabel(r'$\log_{10}(\mathrm{msKID\ value})$', fontsize=25)
    ax.set_title("msKID Distribution", fontsize=30)

    plt.tight_layout()
    plt.savefig("output/hpa/embed/ablation_num_samp_kid_violin.svg", dpi=300, bbox_inches='tight', pad_inches=0, transparent=True)
    plt.close(fig)
    print("saved: output/hpa/embed/ablation_num_samp_kid_violin.svg")

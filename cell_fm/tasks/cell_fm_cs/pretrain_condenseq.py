...
from cell_fm.data.condenseq_data.dataset import CondenSeqDataset
...
@cli(CELLFMConfig)
def main(args) -> None:
    trainset = CondenSeqDataset(args, split_key=args.split_key)
    model = CELLFMCSModel(config=CELLFMConfig(**vars(args)), loss_fn=UniDiffCriterions)
